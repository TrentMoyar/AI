...
class Strategy():
   ...
   def best_strategy(self, board, player, best_move, running):
      ...
      lmoves = legal_moves(myboard, opboard)
      moves = split_binary(lmoves)
      table = {}
      
      best_move.value = get_move(moves[0])
      depth = 1
      if bit_count(myboard | opboard) < 60:
         while True:
            print(table)
            #moves.sort(key=lambda x:self.get_weight(x,myboard,opboard,table), reverse=True)
            best = (None,MIN)
            for move in moves:
               if move & CORNERS & legal_moves(opboard,myboard):
                  best = (move,-3)
                  break
               flipped = flipped_stones(move, myboard, opboard)
               numy, nuop = make_move(myboard, opboard, move, flipped)
               value = self.alpha_beta(numy, nuop, depth, MIN, MAX, False, table)
               if value >= best[1]:
                  best = (move,value)
            if best[1] == -3:
               best_move.value = get_move(best[0])
               break
            best_move.value = get_move(best[0])
            depth += 1
      else:
   ...
   def corner_heur(self,myboard,opboard):
      mycornheur = 2*bit_count(CORNERS & myboard) + bit_count(CORNERS & legal_moves(myboard,opboard))
      opcornheur = 2*bit_count(CORNERS & opboard) + bit_count(CORNERS & legal_moves(opboard,myboard))
   ...
   def evaluate(self, myboard, opboard):
      coin = self.coin_heur(myboard,opboard)
      ...
      return 0.3*corners + 0.3*stability + 0.3*mobility + 0.1*coin
   def alpha_beta(self, myboard, opboard, depth, alpha, beta, maximizing, table):
      if depth <= 0:
         return self.evaluate(myboard, opboard)
      if maximizing:
         moves = legal_moves(myboard, opboard)
         moves = split_binary(moves)
         value = MIN
         #moves.sort(key=lambda x:self.get_weight(x,myboard,opboard,table), reverse=True)
         for move in moves:
            flipped = flipped_stones(move, myboard, opboard)
            newmy, newop = make_move(myboard, opboard, move, flipped)
            alphabeta = self.alpha_beta(newmy, newop, depth - 1, alpha, beta, False, table)
            value = value if value > alphabeta else alphabeta
            alpha = alpha if alpha > value else value
            if value >= beta:
               break
         if depth > 1:
            table[(myboard, opboard)] = value
         return value
      else:
         moves = legal_moves(opboard, myboard)
         moves = split_binary(moves)
         value = MAX
         #moves.sort(key=lambda x:self.get_weight(x,myboard,opboard,table), reverse=True)
         for move in moves:
            flipped = flipped_stones(move, opboard, myboard)
            newop, newmy = make_move(opboard, myboard, move, flipped)
            alphabeta = self.alpha_beta(newmy, newop, depth - 1, alpha, beta, True, table)
            value = value if value < alphabeta else alphabeta
         ...
         if depth > 1:
            table[(myboard, opboard)] = value
         return value
   def alpha_betaf(self, myboard, opboard, alpha, beta, maximizing, table):
      if maximizing:
         moves = legal_moves(myboard, opboard)
         if moves & legal_moves(opboard) == 0:
            return self.coin_heur(myboard, opboard)
         elif legal_moves(board):
            return self.alpha_betaf(myboard,opboard,alpha,beta,False,table)
         moves = split_binary(moves)
         value = MIN
         #moves.sort(key=lambda x:self.get_weight(x,myboard,opboard,table), reverse=True)
         for move in moves:
         ...
         moves = legal_moves(opboard, myboard)
         if moves & legal_moves(myboard) == 0:
            return self.coin_heur(myboard, opboard)
         elif legal_moves(board):
            return self.alpha_betaf(myboard,opboard,alpha,beta,True,table)
         if moves == 0:
            return self.coin_heur(myboard, opboard)
         moves = split_binary(moves)
         value = MAX
