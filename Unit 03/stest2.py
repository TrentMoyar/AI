...
def made_moves(myboard,opboard):
...
class Strategy():
   ...
   def alpha_beta(self, myboard, opboard, depth, alpha, beta):
      table = {}
      moves = made_moves(myboard,opboard)
      v = self.max_value(myboard, opboard, depth, MIN, MAX, table)
      for move in moves:
         if table[move] = v:
            return move
      return -1
   def max_value(self, myboard, opboard, depth, alpha, beta, table):
      mymoves = legal_moves(myboard, opboard)
      opmoves = legal_moves(opboard, myboard)
      if mymoves | opmoves == 0:
         return 1000 if bit_count(myboard) > bit_count(opboard) else -1000
      elif mymoves == 0:
         return self.min_value(myboard,opboard,depth,alpha,beta)
      if depth == 0:
         return self.evaluate(myboard, opboard)
      moves = split_binary(mymoves)
      v = MIN
      for move in moves:
         flipped = flipped_stones(move, myboard, opboard)
         nuboard = make_move(myboard,opboard,move,flipped)
         m = table[nuboard] if nuboard in table else self.min_value(nuboard[0], nuboard[1], depth - 1, alpha, beta)
         v = v if v > m else m
         if v >= beta:
            return v
         alpha = alpha if alpha > v else v
      return v
   def min_value(self, myboard, opboard, depth, alpha, beta, table):
      mymoves = legal_moves(myboard, opboard)
      opmoves = legal_moves(opboard, myboard)
      if mymoves | opmoves == 0:
         return 1000 if bit_count(myboard) > bit_count(opboard) else -1000
      elif opmoves == 0:
         return self.max_value(myboard,opboard,depth,alpha,beta)
      if depth == 0:
         return self.evaluate(myboard, opboard)
      moves = split_binary(opmoves)
      v = MAX
      for move in moves:
         flipped = flipped_stones(move, myboard, opboard)
         nuboard = make_move(myboard,opboard,move,flipped)
