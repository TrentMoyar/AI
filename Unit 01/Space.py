...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   nodecityfile = open(node_city)
   for x in nodecityfile.readlines():
      temp = (x.rstrip('\n')).split(" ",1)
      cityToNode[temp[1]] = temp[0]
      nodeToCity[temp[0]] = temp[1]
   nodecityfile.close()

   nodesfile = open(nodes)
   for x in nodesfile.readlines():
      temp = (x.rstrip('\n')).split(" ")
      nodeLoc[temp[0]] = (temp[1], temp[2])
   nodesfile.close()
   ...
   temp = []
   temploc1, temploc2 = (), ()
   for x in edgefile.readlines():
      temp = (x.rstrip('\n')).split(" ")
      temploc1 = nodeLoc[temp[0]]
      temploc2 = nodeLoc[temp[1]]
      edgeCost[(temp[0], temp[1])] = calc_edge_cost(*temploc1, *temploc2)
      edgeCost[(temp[1], temp[0])] = calc_edge_cost(*temploc1, *temploc2)
      
      if temp[0] not in neighbors:
         neighbors[temp[0]] = [temp[1]]
      else:
         neighbors[temp[0]].append(temp[1])
      
      if temp[1] not in neighbors:
         neighbors[temp[1]] = [temp[0]]
      else:
         neighbors[temp[1]].append(temp[0])
   edgefile.close()
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   for x in range(1, len(path)):
      drawLine(canvas, *graph[5][path[x-1]], *graph[5][path[x]], col)
   # Your code goes here
...
def bi_bfs(start, goal, graph, col):
   ...
   counter = 0
   frontier1,frontier2,explored1, explored2 = deque(), deque(), {start : "s"}, {goal: "s"}
   frontier1.append(start)
   frontier2.append(goal)
   while frontier1 and frontier2:
      s1 = frontier1.popleft()
      s2 = frontier2.popleft()
      if s1 in explored2: 
         path1, cost1 = generate_path(s1, explored1, graph)
         path2, cost2 = generate_path(s1, explored2, graph)
         path = path1 + path2[::-1][1:]
         draw_final_path(ROOT, canvas, path, graph)
         print("bi-BFS1:",len(frontier1))
         print("bi-BFS2:",len(frontier2))
         return path, cost1 + cost2
      if s2 in explored1:
         path1, cost1 = generate_path(s2, explored1, graph)
         path2, cost2 = generate_path(s2, explored2, graph)
         path = path1 + path2[::-1][1:]
         draw_final_path(ROOT, canvas, path, graph)
         print("bi-BFS1:",len(frontier1))
         print("bi-BFS2:",len(frontier2))
         return path, cost1 + cost2
      for a in graph[3][s1]:  #graph[3] is neighbors
         if a not in explored1:
            explored1[a] = s1
            frontier1.append(a)
            drawLine(canvas, *graph[5][s1], *graph[5][a], col)
      for a in graph[3][s2]:  #graph[3] is neighbors
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   heur[start] = heuristic(start, goal, graph)
   s = (heur[start],start)
   frontier.push(s)
   explored = {start : 's'}
   tup = None
   ...
   while not frontier.isEmpty():
      s = frontier.pop()
      if s[1] == goal:
         path, cost = generate_path(s[1], explored, graph)
         draw_final_path(ROOT, canvas, path, graph)
         print("a-star:",len(frontier.queue))
         return path, cost
      #print(s[1])
      for x in graph[3][s[1]]:
         if x not in heur:
            heur[x] = heuristic(x,goal,graph)
         if x not in explored:
            drawLine(canvas, *graph[5][s[1]], *graph[5][x], col)
            tup = (s[0] - heur[s[1]] + graph[4][(x, s[1])] + heur[x], x)
            explored[x] = s[1]
            frontier.push(tup)
            
      counter += 1
      #if counter % 500 == 0: ROOT.update()
   return None

def shared_a_star(start, goal, canvas, ROOT, graph, col, heuristic=dist_heuristic):
   counter = 0
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   heur1[start] = heuristic(start, goal, graph)
   heur2[goal] = heuristic(goal, start, graph)
   s1 = (heur1[start],start)
   s2 = (heur2[goal],goal)
   frontier1.push(s1)
   frontier2.push(s2)
   explored1, explored2 = {start : 's'} , {goal : 's'}

   # Your code goes here
   while (not frontier1.isEmpty()) and (not frontier2.isEmpty()):
      s1 = frontier1.pop()
      s2 = frontier2.pop()
      if s1[1] in explored2:
         path1, cost1 = generate_path(s1[1], explored1, graph)
         path2, cost2 = generate_path(s1[1], explored2, graph)
         path = path1 + path2[::-1][1:]
         draw_final_path(ROOT, canvas, path, graph)
         print("a-star1:",len(frontier1.queue))
         print("a-star2:",len(frontier2.queue))
         return path, (cost1 + cost2)
      if s2[1] in explored1:
         path1, cost1 = generate_path(s2[1], explored1, graph)
         path2, cost2 = generate_path(s2[1], explored2, graph)
         path = path1 + path2[::-1][1:]
         draw_final_path(ROOT, canvas, path, graph)
         print("a-star1:",len(frontier1.queue))
         print("a-star2:",len(frontier2.queue))
         return path, (cost1 + cost2)
      for x in graph[3][s1[1]]:
         if x not in heur1:
            heur1[x] = heuristic(x,goal,graph)
         if x not in explored1:
            drawLine(canvas, *graph[5][s1[1]], *graph[5][x], col)
            tup = (s1[0] - heur1[s1[1]] + graph[4][(x, s1[1])] + heur1[x], x)
            explored1[x] = s1[1]
            frontier1.push(tup)
      for x in graph[3][s2[1]]:
         if x not in heur2:
            heur2[x] = heuristic(x,start,graph)
         if x not in explored2:
            drawLine(canvas, *graph[5][s2[1]], *graph[5][x], col)
            tup = (s2[0] - heur2[s2[1]] + graph[4][(x, s2[1])] + heur2[x], x)
            explored2[x] = s2[1]
            frontier2.push(tup)            
      counter += 1
      #if counter % 100 == 0: ROOT.update()
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
   ...
   ROOT = Tk() #creates new tkinter
   ROOT.title("Tri-Directional A*")
   canvas = Canvas(ROOT, background='black') #sets background
   draw_all_edges(ROOT, canvas, graph)
   
   path = [[] for x in range(3)]
   cost = [0 for x in range(3)]

   path[0], cost[0] = shared_a_star(city1, city2, canvas, ROOT, graph, col)
   path[1], cost[1] = shared_a_star(city2, city3, canvas, ROOT, graph, col)
   path[2], cost[2] = shared_a_star(city3, city1, canvas, ROOT, graph, col)
   
   min = sys.maxsize
   finalpath = []
   for x in range(3):
      if min > (cost[x-1] + cost[x]):
         min = cost[x-1] + cost[x]
         finalpath = path[x-1] + path[x][1:]
   draw_final_path(ROOT, canvas, finalpath, graph)
   # Your code goes here
   return finalpath, min
