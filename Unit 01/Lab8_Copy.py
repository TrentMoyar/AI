...
def a_star(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4): 
   ...
   s1 = (heur1[start],start,0)
   s2 = (heur2[goal],goal,0)
   frontier1.push(s1)
   frontier2.push(s2)
   f1 = set()
   f2 = set()
   f1.add(s1)
   f2.add(s2)
   explored1 = {start : (s1[0], None)}
   explored2 = {goal : (s2[0], None)}
   tup1, tup2 = None, None
   # Your code goes here
   t = 0
   while (not frontier1.isEmpty()) or (not frontier2.isEmpty()):
      ...
      if (s1[1] in f2) or (s1[1] in explored2):
         return tolist(explored1, start, s1[1]) + tolist(explored2, goal, s1[1])[::-1]
